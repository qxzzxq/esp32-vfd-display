...
class UiPage {
  public:
    ...
    // monotonic clock for time-based animation (M6 CUSTOM marquee: position =
    // (now_us / 300 ms) % len — stateless).
    ...
    // when a BMP280 was probed; M6 CUSTOM only when a message is set).
